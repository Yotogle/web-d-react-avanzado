...
const chatReducer = (state, action) => {
  switch (action.type) {
    case 'ADD_MESSAGE':
      ...
      console.log(state)
      return { ...state, message: [...state.messages, action.payload] }
    default:
      return state
  }
}
...
export const App = () => {
  ...
  const handlePregunta = async (data) => {
    ...
    try {
      ...
      // 4 Dispatch para guardar el mensaje del usuario y del bot
      dispatch({
        type: 'ADD_MESSAGE',
  ...
  return (
    <>
      <form onSubmit={handleSubmit(handlePregunta)}>
        ...
        <button className='w-full py-2 rounded transition cursor-pointer bg-blue-600 text-white hover:bg-blue-700'>
          Preguntar
        </button>
      </form>
      <div>
